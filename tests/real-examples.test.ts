...
describe("Real World Examples", () => {
  ...
  describe("1SAT Ordinals Examples", () => {
    test("1SAT token icon", async () => {
      ...
      expect(displayUrl).toBe(
        "https://ordfs.network/b974de563db7ca7a42f421bb8a55c61680417404c661deb7a052773eb24344e3_0",
      );
    });
    ...
    test("GEMS token icon", async () => {
      ...
      const displayUrl = await imageProtocols.getDisplayUrl(url);
      expect(displayUrl).toBe(
        "https://ordfs.network/9418af73d138af02465b22dddd7913660dae9219cd260c4db83cda7c84713896_0",
      );
    });
    ...
    test("GM POW-20 token icon", async () => {
      ...
      const displayUrl = await imageProtocols.getDisplayUrl(url);
      expect(displayUrl).toBe(
        "https://ordfs.network/350ddacd88b98c0ac4590fbc88f1f92ac164cb91187759afcd73f889eea8bef7_0",
      );
    });
  });
  ...
  describe("Bitcoin Files Examples", () => {
    test("Rare Sirloins #2149", async () => {
      ...
      const displayUrl = await imageProtocols.getDisplayUrl(url);
      expect(displayUrl).toBe(
        "https://ordfs.network/cbacb16c3a03729165542f20404827f1ee91bc1f9783089c41c59524ebf75a22_0",
      );
    });
  });
  ...
  describe("BitFS Examples", () => {
    test("BSocial post image", async () => {
      ...
      const displayUrl = await imageProtocols.getDisplayUrl(url);
      expect(displayUrl).toBe(
        "https://ordfs.network/868e663652556fa133878539b6c65093e36bef1a6497e511bdf0655b2ce1c935_0",
      );
    });
  });
  ...
  describe("Native Format Examples", () => {
    test("Pixel Fox Collection ID", async () => {
      ...
      const displayUrl = await imageProtocols.getDisplayUrl(url);
      expect(displayUrl).toBe(
        "https://ordfs.network/1611d956f397caa80b56bc148b4bce87b54f39b234aeca4668b4d5a7785eb9fa_0",
      );
    });
    ...
    test("Content path with txid", async () => {
      ...
      const displayUrl = await imageProtocols.getDisplayUrl(url);
      expect(displayUrl).toBe(
        "https://ordfs.network/e6d40ba206340aa94ed40fe1a8adcd722c08c9438b2c1dd16b4527d561e848a2_0",
      );
    });
    ...
    test("Content path with txid_vout", async () => {
      ...
      const displayUrl = await imageProtocols.getDisplayUrl(url);
      expect(displayUrl).toBe(
        "https://ordfs.network/1611d956f397caa80b56bc148b4bce87b54f39b234aeca4668b4d5a7785eb9fa_2",
      );
    });
    ...
    test("Dot notation format", async () => {
      ...
      const displayUrl = await imageProtocols.getDisplayUrl(url);
      expect(displayUrl).toBe(
        "https://ordfs.network/1611d956f397caa80b56bc148b4bce87b54f39b234aeca4668b4d5a7785eb9fa_3",
      );
    });
    ...
    test("Bitcoin-style output notation (txido0)", async () => {
      ...
      const displayUrl = await imageProtocols.getDisplayUrl(url);
      expect(displayUrl).toBe(
        "https://ordfs.network/1611d956f397caa80b56bc148b4bce87b54f39b234aeca4668b4d5a7785eb9fa_1",
      );
    });
  ...
  describe("Integration with Different Gateways", () => {
    test("custom gateway for token resolution", async () => {
      ...
      expect(displayUrl).toBe(
        "https://tokens.1satordinals.com/api/icon/a54d3af24a03bcc28f6b3f2dd0ad249ee042b2f4b95810ae5184ab617a74b8b9_0",
      );
    });
  });
});
